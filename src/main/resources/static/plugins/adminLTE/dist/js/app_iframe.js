...
var App = function () {
    ...
    return {
        ...
        blockUI: function (options) {
            console.log("blockUI-function: " + JSON.stringify(options));
...
function canRemoveTab(pageId) {
    return findTabTitle(pageId).find('.fa-times-circle').length > 0;
}
...
var addTabs = function (options) {
    ...
    if (findTabPanel(pageId) === null) {
        ...
        //是否允许关闭
        if (options.close) {
            var $i = $("<i class='fas fa-times-circle page_tab_close' style='cursor: pointer' onclick='closeTab(this);'></i>").attr(pageIdField, pageId).appendTo($title);
        ...
        if (options.content) {
            ...
            console.log("输入的url: " + options.url);
...
!function () {
    ...
    function e(e) {
        ...
        function o(t, o) {
            var s, l = t == window, a = e(t), d = a.data("blockUI.history"),
        ...
        function s(t) {
            if ("keydown" === t.type && t.keyCode && 9 == t.keyCode && p && t.data.constrainTabKey) {
                var o = b, n = !t.shiftKey && t.target === o[o.length - 1],
...
(function ($) {
    $.fn.sidebarMenu = function (options) {
        ...
        $menu_ul.on("click", "li.treeview a", function () {
            // TODO 扩充事件
            var $a = $(this);
            //console.log("下一级标签："+$.AdminLTE.options);// $a.next().size() == 0
            if ($a.next().length) {//如果size>0,就认为它是可以展开的
                //触发左边菜单栏按钮点击事件,关闭菜单栏
